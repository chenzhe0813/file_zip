								...
								<div class="scripts-choosen-box">
									...
									<div class="scripts-choosen clearfix" v-for="step in scriptSteps">
										...
										<div class="scripts-files">
											<div v-for="(item, index) in scriptsChoosen[step.key]">
												<p class="scripts-files-name">{{item.name}}</p>
												<p class="scripts-files-desc">{{item.desc}}</p>
												<p class="scripts-files-operate">
													<span @click="scriptsDelete(index, scriptsChoosen[step.key])">删除</span>
													<span @click="scriptsEdit(item, index, step.key)">修改</span>
												</p>
											</div>
										</div>
									</div>
		...
		addScriptSubmit(){
			...
			if(_this.dialogShowCheckbox){
				_this.dialogScriptForm.steps.map((item)=>{
					_this.scriptsChoosen[item].push(data);
				})
			}else{
				// console.log(_this.scriptsChoosen[_this.editFileToStep.step][_this.editFileToStep.index])
				_this.scriptsChoosen[_this.editFileToStep.step][_this.editFileToStep.index] = data;
			}
